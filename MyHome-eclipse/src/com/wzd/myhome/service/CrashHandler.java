...
import com.zzy.app.util.FileUtil;
import com.zzy.app.util.Log4JUtil;
import com.zzy.app.util.ZzyUtil;

/**
 * 类名 CrashHandler 实现的主要功能 程序奔溃操作类，内部类，实现UncaughtExceptionHandler接口
 */
public class CrashHandler implements UncaughtExceptionHandler {
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		...
		if (!handleException(ex) && mDefaultHandler != null) {
			...
			if (AndroidUtil.isClientRunTop(mContext)) {
				Log4JUtil.info("go to this");
				// Intent appIntent = NotificationHelper
				// .getNotifyIntent(mContext);
				// mContext.startActivity(appIntent);
				// System.exit(0);
				// Intent splashIntent = new Intent(mContext,
	...
	private boolean handleException(final Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				...
				dumpString(ZzyUtil.dumpThrowable(ex), mContext);
				Looper.loop();
				Lock = false;
			}
